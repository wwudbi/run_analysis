run_analysis <- function() {
  ...
  subset_data <- merge(subset_data,activity_label,by.x="activity",by.y="ActivityId",all=FALSE) 

  ## STEP 4: Appropriately labels the data set with descriptive variable names  
  ## This step has already completed when read the files
  ## Delete redundent activity column; Set subject and ActivityName columns as factor
  
  subset_data$activity <- NULL
  subset_data$subject <- as.factor(subset_data$subject)
  subset_data$ActivityName <- as.factor(subset_data$ActivityName)
  
  ## STEP 5: From the data set in step 4, creates a second, independent tidy data set with 
  ##  the average of each variable for each activity and each subject  
  library(reshape2)
  
  ## Incorrect result using aggregate function
  tidy_data <- aggregate(subset_data[2:80],by=list(subset_data$subject,subset_data$ActivityName),FUN=mean)
